...
/**
 * Reads the current URL query string client-side without triggering a hydration
 * mismatch: the server snapshot is empty (matching the statically-prerendered
 * HTML) and the client snapshot reads `window.location.search` after hydration.
 * The query string only changes via navigation (which remounts), so no live
 ...
 * ISR-cacheable (#982). The OAuth error banner appears after hydration on the
 * rare post-redirect path — an acceptable trade for keeping the highest-traffic
 * route on the CDN.
 */
function LandingUrlEffects() {
