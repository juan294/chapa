...
describe("POST /api/auth/bitbucket/disconnect contract", () => {
  it("deletes the linked platform and returns the write result", async () => {
    const response = await invokeJson(POST, {
      method: "POST",
      path: "/api/auth/bitbucket/disconnect",
      body: {},
    });

    expect(response.status).toBe(200);
    expect(bodyAsRecord(response).success).toBe(true);
    expect(mockDbDeleteLinkedPlatform).toHaveBeenCalledWith("octocat", "bitbucket");
