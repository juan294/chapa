...
const mocks = vi.hoisted(() => ({ auth: vi.fn(), write: vi.fn(), read: vi.fn(), artifact: vi.fn(), limit: vi.fn(), invalidate: vi.fn(), del: vi.fn(), capture: vi.fn() }));
...
beforeEach(() => { vi.clearAllMocks(); mocks.limit.mockResolvedValue({ allowed: true }); mocks.auth.mockResolvedValue({ handle: "owner" }); mocks.write.mockResolvedValue({ success: true }); mocks.read.mockResolvedValue({ ownerId: "owner", claims: [] }); mocks.invalidate.mockResolvedValue(undefined); mocks.del.mockResolvedValue(true); });
const request = (body: unknown) => new NextRequest("http://localhost/api/evidence", { method: "POST", body: JSON.stringify(body) });
const retract = (owner: string) => ({ action: "retract", owner, revisionId: "00000000-0000-4000-8000-000000000000", rationale: "no longer accurate" });
describe("private evidence route boundary", () => {
  ...
  it("denies owner forgery before persistence", async () => {
    expect((await POST(request(retract("other")))).status).toBe(403);
    expect(mocks.write).not.toHaveBeenCalled();
  });
  it("retires the withdraw action without reaching the ledger or invalidation", async () => {
    const response = await POST(request({ action: "withdraw", owner: "owner" }));
    expect(response.status).toBe(400);
    expect(await response.json()).toEqual({ error: "retired_action" });
    expect(response.headers.get("cache-control")).toBe("private, no-store");
    expect(mocks.write).not.toHaveBeenCalled();
    expect(mocks.invalidate).not.toHaveBeenCalled();
  });
  it("no longer recognizes the retired consent action at all", async () => {
    const response = await POST(request({ action: "consent", owner: "owner", enabled: true, publicationAcknowledged: true }));
    expect(response.status).toBe(400);
    expect(mocks.write).not.toHaveBeenCalled();
  });
  it("rejects oversized and undeclared trusted properties", async () => {
    expect((await POST(request({ action: "withdraw", owner: "owner", authorization: "forged" }))).status).toBe(400);
  ...
  it("maps denied reads and revision conflicts without leaking database details", async () => {
    ...
    mocks.write.mockRejectedValue(new LedgerStorageError("40001"));
    expect((await POST(request(retract("owner")))).status).toBe(409);
  });
