...
// [locale] route param (populated by proxy.ts from the resolved
// locale) drives getServerT(locale) directly in the initial render — no
// client-side re-render is needed to show Spanish content.
describe("AboutPage render (es) — locale-segmented RSC, no client re-render", () => {
  it("renders the about page with es heading", async () => {
    ...
    const { default: AboutPage } = await import("./page");
    render(await AboutPage({ params: Promise.resolve({ locale: "es" }) }));
...
describe("VerificationPage render (es)", () => {
  it("renders the verification page heading in es", async () => {
    ...
    const { default: VerificationPage } = await import("./verification/page");
    render(await VerificationPage({ params: Promise.resolve({ locale: "es" }) }));
