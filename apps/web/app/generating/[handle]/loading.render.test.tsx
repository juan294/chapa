...
// dictionary, not a hardcoded English literal. `app/generating/[handle]/page.tsx`
// is `force-dynamic` and resolves the real per-request locale via
// getServerLocale(); this loading.tsx mirrors that instead of assuming
// DEFAULT_LOCALE.
describe("GeneratingLoading render — i18n (#1109)", () => {
  beforeEach(() => {
  ...
  it("resolves the real per-request locale via getServerLocale(), not a hardcoded default", async () => {
    ...
    const { default: GeneratingLoading } = await import("./loading");
