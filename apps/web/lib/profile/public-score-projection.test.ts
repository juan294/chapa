...
it.each([57, 0] as const)("projects canonical current values and Craft%s without legacy traps", async craft => {
  ...
  const result = publicScoreProjection(f.model);
...
it("compares only compatible policies and observation periods, preserving both contexts", async () => {
  ...
  // The archived machine-v7 policy carries different aggregate semantics than
  // the current v7.2 policy (#1335 phase 5 — there is no v6 to compare against
  // any more; the mismatch this test proves is v7-vs-v7.2).
  const archived = publicScoreProjection({ ...f.model, policyVersion: "v7" as const });
  expect(comparePublicScores(a, archived)).toMatchObject({ status: "not_comparable", reason: "policy_mismatch", differences: null });
