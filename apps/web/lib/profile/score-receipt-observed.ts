...
import { createScoringWindow, evidenceSourceKey, projectReceiptEvidence, scoringInstant, sealObservedScoreReceipt, RECEIPT_ALGORITHM_OBSERVED, SCORING_OBSERVED_POLICY,
...
/** Additive write orchestrator, intentionally not called by the active scorer.
...
export async function materializeObservedScoreReceipt(owner: string, options: ObservedReceiptMaterializationOptions = {}): Promise<ObservedReceiptMaterialization> {
  ...
  try {
    ...
    const window = createScoringWindow(correction?.envelope.receipt.window.referenceTime ?? options.referenceTime ?? new Date().toISOString());
    ...
    if (collected.sources.some(source => source.status === "unavailable" || source.status === "stale" || source.status === "legacy" || source.reasonCodes.includes("source_error"))) return preserve("source_error");
    ...
    const evidence: EngineeringEvidenceInput = canonicalizeReceiptEvidence({ schemaVersion: "v7", window,
      scope: { sources: [...collected.sources, ...ledger.scope.sources], excludedSources: [...collected.excludedSources, ...ledger.scope.excludedSources], ledgerRevisionIds: ledger.scope.ledgerRevisionIds },
    ...
    const semanticDigest = await receiptSemanticIdentity(envelope.receipt, evidence, ledgerSnapshot);
    ...
    const published = await dbPublishObservedReceipt(handle, handle, unchanged ? baseline.envelope : envelope, semanticDigest);
