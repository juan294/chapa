 ...
 */
/**
 * The OAuth scopes Chapa requests at login.
 *
 * Deliberately excludes `repo`. Chapa is a public badge product; asking every
 * user to grant full read/write access to all their private repositories to
 * render a score would be a disproportionate demand. That is a product
 * decision, not an oversight — but it has a consequence the scoring pipeline
 * must respect, which is why this is exported rather than inlined (#1050).
 */
export const OAUTH_SCOPES = "read:user user:email";

/**
 * Whether a session OAuth token can see the user's PRIVATE repositories.
 *
 * False, because {@link OAUTH_SCOPES} omits `repo`. This is load-bearing for
 * `fetchScope` in `lib/github/client.ts`: without `repo`, a user's own session
 * token sees strictly less than an anonymous request that falls back to the
 * server `GITHUB_TOKEN`. Labelling such a fetch "authenticated" is what let a
 * user's own refresh overwrite complete data with a scope-blinded view of it
 * (#1050 — juan294 saw 140 of 987 merged PRs, and Delivery fell 100 -> 58).
 *
 * Derived from the scope string rather than hardcoded, so that if `repo` is
 * ever added to {@link OAUTH_SCOPES} the pipeline's trust in session tokens
 * updates in the same edit instead of silently drifting out of sync.
 */
