...
/** Fetch heatmap — single call, no pagination. Returns null on auth failure. */
async function fetchHeatmap(
  username: string,
  token: string,
  signal: AbortSignal,
): Promise<CodebergHeatmapEntry[] | null> {
  const res = await fetch(`${CB_API}/users/${encodeURIComponent(username)}/heatmap`, {
    headers: { Authorization: `token ${token}`, Accept: "application/json" },
    signal,
  });
  ...
  return (await res.json()) as CodebergHeatmapEntry[];
}

/** Fetch user repos (paginated, capped at MAX_REPOS) */
...
/** Fetch merged PRs authored by username in a repo */
...
/** Fetch reviews by username on other people's PRs */
...
/** Fetch closed issues authored/assigned to username in a repo */
