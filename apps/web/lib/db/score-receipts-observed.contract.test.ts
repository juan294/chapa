...
describe("v7.2 atomic publication and policy history", () => {
  ...
  it("has exact service-only RPC and current-pointer grants", () => {
    expect(inspectLocalSql("SELECT p.proname,has_function_privilege('anon',p.oid,'EXECUTE'),has_function_privilege('authenticated',p.oid,'EXECUTE'),has_function_privilege('service_role',p.oid,'EXECUTE') FROM pg_proc p JOIN pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname='public' AND p.proname IN ('scoring_observed_publish_receipt','scoring_observed_receipt_manifest','scoring_observed_read_receipt') ORDER BY p.proname").split("\n")).toEqual(["scoring_observed_publish_receipt|f|f|t", "scoring_observed_read_receipt|f|f|t", "scoring_observed_receipt_manifest|f|f|t"]);
