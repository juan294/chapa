...
interface CraftSnapshot {
  ...
  references: { reference_id: string; owner_handle: string; retention: string }[];
...
export async function dbReadCraftV7(owner: string, actor: string, window: ScoringWindow) {
  ...
  for (const row of snapshot.evidence) {
    ...
    claims.push({ ...claim, recordedAt: new Date(row.recorded_at).toISOString() });
  }
  const grants = new Map(snapshot.grants.map(grant => [grant.reviewer_handle, grant]));
  const assessments = snapshot.assessments.map(assessmentFromRow).filter(assessment => {
    ...
    return grant && Date.parse(grant.granted_at) <= Date.parse(assessment.recordedAt) && Date.parse(grant.granted_at) <= Date.parse(assessment.assessedAt) &&
      (!grant.revoked_at || Date.parse(grant.revoked_at) > Date.parse(assessment.recordedAt));
  });
  const dated = new Map(snapshot.evidence.filter(row => row.occurred_at !== null).map(row => [row.id, row.occurred_at!]));
  const episodes: CraftEpisode[] = claims.filter(claim => dated.has(claim.revisionId)).map(claim => ({ episodeId: claim.revisionId, workItemId: claim.workItemId,
    occurredAt: dated.get(claim.revisionId)!, artifactRevision: claim.artifactRevision, evidenceReferenceIds: claim.evidenceReferenceIds,
  ...
  const inputs = buildCraftInputs({ ownerId: owner.toLowerCase(), window, claims, portfolios,
    authorizedEvaluatorIds: [...grants.keys()], referenceIds: snapshot.references.filter(ref => ref.owner_handle === owner.toLowerCase() && ref.retention === "until_owner_withdrawal").map(ref => ref.reference_id) });
