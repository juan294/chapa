...
// #1335 — v7.2 is the one scoring policy; every import publishes a
// report-derived Craft observation through uploadObserved (one /api/insights
// call), never the retired v6 upload+recalculate flow.
describe("insights notification identity and reload (#1292)", () => {
  ...
  it("assigns processing and success distinct ids and reloads only 2.5s after completion", async () => {
    ...
    await act(async () => {
      finishUpload(new Response(JSON.stringify(published(72)), { status: 200 }));
      await pending;
    });
    expect(result.current.toast!.type).toBe("success");
    expect(result.current.toast!.id).toBe(processingId + 1);
  ...
  it("cancels the completion reload when Settings unmounts", async () => {
    vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(new Response(JSON.stringify(published(72)), { status: 200 }));
...
describe("report-derived Craft import", () => {
  it("discards a pending confirmation after an account change", async () => {
    ...
    const { result, rerender } = renderHook(({ login }) => useInsightsImport(login), { initialProps: { login: "alice" } });
  ...
  it("allows only one import while the selected file is still being read", async () => {
    ...
    const { result } = renderHook(() => useInsightsImport("alice"));
    let pending!: Promise<void>;
