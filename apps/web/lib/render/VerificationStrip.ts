...
export function renderVerificationStrip(hash: string, date: string): string {
  ...
  <text transform="rotate(-90 ${centerX} ${textY})" x="${centerX}" y="${textY}" font-family="'JetBrains Mono', monospace" font-size="11" fill="${CORAL}" opacity="0.50" text-anchor="middle" letter-spacing="2">VERIFIED \u00B7 ${safeHash} \u00B7 ${safeDate}</text>
</g>`;
}
