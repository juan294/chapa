 ...
 *
 * @param verifiedLabel - Locale-resolved "VERIFIED" label. #1181 (UX-H3) —
 *   resolved by the caller via `getServerT`; defaults to English so this
 *   function stays pure/sync and existing (unmigrated) callers are unaffected.
 */
export function renderVerificationStrip(
  hash: string,
  date: string,
 ...
 *
 * @param sampleDisclosure - Locale-resolved disclosure text (the whole
 *   "SAMPLE · NOT A REAL BADGE · FOR ILLUSTRATION ONLY" line). #1181 (UX-H3)
 *   — resolved by the caller; defaults to English for existing callers.
 */
export function renderDemoVerificationStrip(
  sampleDisclosure: string = DEFAULT_SAMPLE_DISCLOSURE,
): string {
  const lineX = 1145;
  const centerX = 1168;
  const textY = 315;
