...
import { fetchGitlabEvidence } from "./evidence";
...
describe("GitLab v7 evidence", () => {
  ...
  it("does not convert failed diff fetching into measured zero", async () => {
  ...
  it("processes more than ten review candidates", async () => {
    ...
    const result = await fetchGitlabEvidence(7, "alice", "token", window, { maxRequests: 100 });
    expect(result.events.filter((e) => e.kind === "review")).toHaveLength(12);
  });
  it("reports unavailable review endpoints as unavailable, never complete zero", async () => {
    api((url) => url.pathname.endsWith("/notes") ? json({}, 403) : undefined);
    const result = await fetchGitlabEvidence(7, "alice", "token", window, { repositoryIds: [10] });
    expect(result.coverage.eventKinds.review).toBe("unavailable");
    expect(result.coverage.reasonCodes).toContain("not_accessible");
  });
  it("retains preceding pages and incoming resume page after a failure", async () => {
    api((url) => {
      if (url.pathname !== "/api/v4/merge_requests") return;
      return url.searchParams.get("page") === "1" ? json([mr()], 200, { "x-next-page": "2" }) : json({}, 500);
    });
    const result = await fetchGitlabEvidence(7, "alice", "token", window);
    const progress = result.progress.find((p) => p.path === "/merge_requests")!;
    expect(progress).toMatchObject({ complete: false, nextPage: 2, collectedNodes: 1 });
    expect(result.events.some((e) => e.kind === "accepted_change")).toBe(true);
  });
  it("enforces one request budget across collectors and records unfinished work", async () => {
    api(); const result = await fetchGitlabEvidence(7, "alice", "token", window, { maxRequests: 6 });
  ...
  it("keeps absent account email permission from becoming a complete commit zero", async () => {
    api((url) => {
    ...
    expect(result.coverage.eventKinds.authored_commit).toBe("partial");
  ...
  it("keeps acceptance time unknown when an old-authored commit may first land inside the window", async () => {
    api((url) => {
      ...
      if (url.pathname.endsWith("/repository/commits")) return json([
        { id: "old-authored", author_email: "alice@example.test", authored_date: "2024-01-01T00:00:00Z", committed_date: date },
      ]);
    });
    const result = await fetchGitlabEvidence(7, "alice", "token", window, { repositoryIds: [10] });
  ...
  it("rejects credentials for a different subject before attributing emails", async () => {
    api(); await expect(fetchGitlabEvidence(8, "bob", "token", window)).rejects.toThrow("subject");
