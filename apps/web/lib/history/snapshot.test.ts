...
describe("buildSnapshot", () => {
  ...
  it("excludes heatmapData, confidencePenalties, displayName, avatarUrl", () => {
    const snapshot = buildSnapshot(makeStats(), makeImpact());
    const keys = Object.keys(snapshot);

    expect(keys).not.toContain("heatmapData");
    expect(keys).not.toContain("confidencePenalties");
    ...
    expect(keys).not.toContain("maxCommitsIn10Min");
    expect(keys).not.toContain("microCommitRatio");
    expect(keys).not.toContain("docsOnlyPrRatio");
  });
  ...
  it("has exactly 24 keys", () => {
    const snapshot = buildSnapshot(makeStats(), makeImpact());
    // 2 metadata + 13 stats + 10 impact = 24 (breadth is counted once under impact)
    // Actually: date, capturedAt, commitsTotal, prsMergedCount, prsMergedWeight,
    // reviewsSubmittedCount, issuesClosedCount, reposContributed, activeDays,
    // linesAdded, linesDeleted, totalStars, totalForks, totalWatchers, topRepoShare,
    // building, guarding, consistency, breadth, archetype, profileType,
    // compositeScore, adjustedComposite, confidence, tier = 25
