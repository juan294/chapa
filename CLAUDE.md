...
- **Read-only cold-key reads (#1083)**: `getStats({ readOnly: true })` (used by `/api/profile/:handle` and other read-only callers) on a cache miss past the 6h TTL composes strictly from the protected `stats:stale:v2:<handle>` baseline via `_composeFromBaselineOnly()` — it never triggers a live GitHub fetch. Previously every cold-key hit on a public read endpoint forced a full GitHub fetch, which could hammer GitHub's API on a high-traffic handle. Separately, `_loadOverlays` now fetches each linked platform's DB link row once per wave instead of a second serialized wave (#1093/PE-L2), and in `_fetchAndCache` the GitHub GraphQL fetch now runs concurrently with `_loadOverlays` and the supplemental lookup (#1087/PE-H2) — the integrity guards below still only ever see `primary`/`baseline`, never `overlays`.
...
3. Release to production via `develop` → `main` squash PR, following
   ...
   — squashing discards shared git history, so `.github/workflows/auto-backmerge.yml`
   automatically merges `main` back into `develop` (strategy `ours`, preserves
   `develop`'s tree byte-for-byte) on every push to `main`, keeping the *next*
   release PR's merge-base clean. Without it, a release PR can silently come
   back `CONFLICTING` and skip every `pull_request`-triggered CI check,
   including the pending-migrations gate — this happened 3 times manually
