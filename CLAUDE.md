...
## One-liner
Chapa generates a **live, embeddable, animated SVG badge** showing recorded v7.2 evidence (four core dimensions and optional report-derived Craft) for every signed-up user, with a Creator Studio for badge preview customization, a share page, and one-click sharing.

This branch implements universal v7.2 scoring with durable, resumable evidence
collection (#1335): every signed-up subject is scored, with no publication
consent step, and a durable job queue instead of a single collection attempt.
Production changes to this behavior only on release, following
`docs/release/release-playbook.md`; implementation and local proof authorize
neither the release nor the production migration that retires the legacy
tables (`docs/decisions/2026-09-23-universal-v72-no-consent.md`).
...
2. Compute the current **Impact v7.2 observed point profile** over 365 UTC dates.
   ...
   Archived machine v7/algorithm v7.1 engines/receipts remain immutable. There is
   one current policy, `SCORING_POLICY = "v7.2"` (`packages/shared`); no rendered
   or selectable v6 remains anywhere (#1335). Every signed-up subject (a handle
   with a `user_platforms` row for `github`, #1239) is scored with no
   publication-consent step; a durable, resumable job queue collects evidence
   across cron ticks instead of one attempt, and the first receipt is not
   published until that collection completes. Failed authority reads are
   unavailable.
...
6. Badge **verification** via a `v7.` receipt token (proves badge data hasn't been tampered with). A well-formed legacy pre-v7 hash is recognized and answers HTTP 410 `retired_v6_code`, not a lookup; `verification_records` and v6 HMAC codes no longer exist.
...
9. Show the top three distinct canonical displayed scores on the landing page.
   v7.2 considers every registered subject with a drawable current receipt,
   skips subjects still collecting or with no receipt, and groups equal scores
   into one place with alphabetical handles. Landing rendering is dynamic;
...
- GET `/api/health` Health check (Redis dbsize + Supabase query + GitHub API probe + cron heartbeat staleness for warm-cache/sync-audience/process-campaigns/latency-check/collect-evidence, rate-limited; returns "skipped" for unconfigured services; #1047 — asserts the server `GITHUB_TOKEN` still carries `repo` scope and returns a distinct `insufficient_scope` status if not, since that token silently losing `repo` would blind every badge to private-repo merges with no other signal). Also reports a `scoringQueue` block (#1335): the collection queue's stuck/failed counts plus the `collect-evidence` cron heartbeat, degraded when either looks stalled.
...
- GET|POST `/api/scoring/status` Owner-only read of the authenticated handle's current collection/receipt status (ready, collecting, action needed, unregistered); POST `{action:"retry",provider}` re-enqueues a failed provider's job and runs one slice in the background (#1335)
...
- GET `/api/cron/collect-evidence` Every-5-minute durable evidence collection worker (bearer auth via `CRON_SECRET`; `maxDuration` 300s; #1335) — claims leased jobs from `scoring_collection_jobs`, runs one checkpointed collector slice per job per tick, and fans in issuance once all of an owner's connected providers finish a day; see `docs/runbooks/scoring-collection-queue.md`
...
- `SCORING_POLICY` — the constant `"v7.2"`, the single current machine policy (`packages/shared/src/scoring-policy.ts`, #1335). `ImpactV6Result` survives only as an archived union member inside the byte-digested `packages/shared/src/scoring-evidence.ts` and its re-export from `types.ts`/`index.ts`; nothing else reads v6 result shapes.
- `BadgeConfig` — Creator Studio visual customization (7 categories, all of which reach the SVG badge; `RETIRED_BADGE_CONFIG_KEYS` names the three dropped in #1191 and must be stripped from a persisted config before validation, and `withDefaultBadgeConfigKeys` fills in any category added since it was saved — `isValidBadgeConfig` requires an exact key set, so a new category silently invalidates every stored config without it, #1242)
...
- Default badge design: **`ice-terminal-v2`**, with Ice ground `#0C141B`, accent `#BAD9E8` and a clean JetBrains Mono identity header. `WARM_AMBER` is the historical export name for the current Ice default. Geometry, typography and static/animated behavior are documented in `docs/svg-design.md`.
- **Badge palettes**: `BadgeConfig.colorPalette` (alias `palette` for `/set`) selects one of six color directions: Ice, Jade, Indigo, Amber, Crimson or Mono. `badgeTheme(palette)` owns each accent/ground pair. New, no-row and reset configs use Ice; persisted rows missing `colorPalette` normalize to Jade after retired-key stripping and legacy alias renaming, while explicit saved palette values retain their colors. Schema stays at seven fields; unavailable/invalid config reads remain non-cacheable. The versioned layout changes globally, including saved Jade badges; there is no selectable historical layout.
...
- **The stats cache is bound to the grant that produced it (`lib/cache/stats-cache.ts`)**: S08 (#1303) deleted the old `stats:v2:merged:<handle>` cache because a bare-handle key carried no authorization binding, so a fetch made with one credential could be served to a caller holding another. It left `getStats` with no cache at all, and every render then paid a live GitHub fetch: the `CONTRIBUTION_QUERY` measures ~9-10s per handle against a 15s `AbortSignal.timeout`, which is what turned the leaderboard's serial materialize loop into a 30-45s landing page and made the badge route's 4100ms cache-miss SLO unreachable. The replacement key is `stats:v3:<handle>` and **the binding lives in the value, not in the key**: a domain-separated HMAC over the access context and the linked-grant versions, which `readCachedStats` must match before it will serve a row. **The binding deliberately excludes the scoring day (badge-source-outage-resilience, 2026-09-22)**: the envelope carries its own `referenceDate`, `capturedAt` and `freshUntil` alongside the binding, so a same-grant record written yesterday is still exactly bound today instead of becoming an unrelated miss across a UTC-day rollover. A read is `fresh` only on a same-day hit inside a 6h window after capture; any exactly-bound record stays eligible as `stale` for up to 7 days after capture, and only when a recheck at read time finds the caller's current authorization state byte-for-byte the same as what wrote the entry. A `stale` read can never persist a snapshot, mint a verification record, or write the normal 24h SVG cache. One key per handle keeps invalidation a single `cacheDel` (`buildStatsCacheKey`, used by `invalidateProfileReadModels` and `platform-oauth.ts`); binding in the key instead would need a per-handle index or an epoch counter and a second round trip on every read. The cost is that one handle holds one binding, so an owner's session-token fetch and the badge route's server-token fetch evict each other, and each still pays at most one live fetch while neither is ever served the other's data. **The in-flight coalescing key stays day-scoped even though the cache binding is not.** `PrivateSourceContext.selectionId` HMACs the exact reference *instant*, so a key built from it alone is unique per call; the in-flight map appends the scoring day explicitly, so two concurrent calls for one grant on the same day still share a fetch while two calls for the same grant on different scoring days collect two independent 365-day windows instead of one sharing the other's.
- Cache SVG output per user/day + theme + locale (TTL 24h + per-handle jitter of 0–2h to spread UTC-midnight recompute spikes)
- **Durable evidence collection queue (#1335)**: `scoring_collection_jobs` (migration 055) is a lease-based job table, one row per `(owner, provider, reference_date)`, worked by the `collect-evidence` cron every 5 minutes (`lib/collection/worker.ts`). Each tick claims leased jobs and runs one checkpointed collector slice per job against a time/request budget; a rate limit waits until the provider's reset time, a transient error backs off across 8 attempts, and a per-item 401/403/404 (one deleted PR, one now-private file list) is absorbed as partial coverage rather than failing the job, while a 401/403 on the credential itself is terminal and recoverable only by reconnecting. Migrations 061-063 store up to 100,000 events per source in immutable generation rows, with 1,000-event checkpoints and verified 500-event reads; finish returns a small manifest. A finish or checkpoint storage fault uses bounded lease-fenced retries and preserves staged progress. `event_limit` is terminal and cannot be requeued by ordinary owner Retry; operator action is required. `scoring_status(owner, referenceDate)` (migration 056) reports `ready` / `collecting` / `action_needed` / `unregistered` per owner; fan-in issues the receipt once every connected provider's job for the day is `complete`. `lib/collection/queue-health.ts` defines the "stuck" thresholds (oldest queued job >2h, or a lease expired >30min) shared by the worker's `scoring_queue_stuck` P2 alert and `/api/health`'s `scoringQueue.degraded`. Enqueueing (`enqueueCollection`) happens only from the authenticated/durable-write side of the app — OAuth callback, owner refresh/recalculate/generate, platform reconnect, warm-cache, or admin — never from a public read, per the #1239 boundary below. **Retry attempt policy (migration 058, #1351)**: `attempt` means "failures since the last progress", not "failures ever". A `rate_limited` stop never increments it, and a checkpoint that advances `operationsDone` resets it to 0, so a job that only waits for the shared allowance can never exhaust its retry budget and a long job with an occasional transient error keeps its progress instead of restarting. **Discovery-aware progress (#1342)**: while any in-progress job's operation count can still grow (`progress.discovering`, carried from the collector's own `discoveryComplete` flag), every scored surface shows "discovering" instead of a percentage, so the displayed number never moves backwards once it appears.
...
- Badge and OG response freshness is at most300 seconds, capped by elapsed
  ...
  Image keys/locks/URLversions include the `v7.2` policy segment; exact receipt manifests
  ...
  assumptions. See `docs/runbooks/scoring-collection-queue.md`.
...
- Leftover DOM effects: `apps/web/lib/effects/*` — no longer Studio's, since the customization categories are rendered by `lib/render/badge-effects.ts` into the SVG (#1191, step 6). What survives here does so for a different consumer: `counters/` and `text/ScoreEffectText` + `tier/TierVisuals` for the dashboard, `heatmap/animations`, `interactions/use-tilt` and `celebrations/confetti` for their `/experiments/*` prototypes, and `defaults.ts` for the Studio presets. The DOM background, border, card-style and heatmap-grid implementations were deleted with `BadgeContent`.
- Creator Studio UI: `apps/web/app/studio/*` — authenticated owner preview (the real badge SVG), controls, save state, and command actions. `page.tsx` resolves the badge avatar server-side against a bounded deadline, as the badge route does, so the preview draws the owner's photo rather than the shield placeholder. **Layout is a horizontal split (#1241)**: a full-width stage on top (badge, `% chapa studio preview` header, Fit/50%/100% zoom, save pill, one-line config summary + `/copy config`), then a tools band that splits on its own width (`repeat(auto-fit, minmax(min(100%,460px),1fr))`) with Quick Controls left and the session log right. Three things that layout depends on: the 50%/100% badge frames are flex children of a horizontally scrolling viewport and need `shrink-0` or flexbox silently shrinks them back to the container width; zoom is presentation only and must never reach the saved config; and `/save` + `/reset` live in the session column, not inside the collapsible Quick Controls panel, which used to hide the only pointer route to saving. The config summary and its `/set` command form are one pure module, `studio-config-string.ts`
...
- WebMCP (agent-facing tools): `apps/web/lib/webmcp/*` — the registration adapter (`use-model-context-tools.ts`), tools shared by more than one page (`shared-tools.ts`), and the published route/tool map (`site-tool-map.ts`). Per-page registrations live beside the page they belong to: `components/LandingWebMcpTools.tsx`, `app/studio/useStudioWebMcpTools.ts`, `app/u/[handle]/SharePageWebMcpTools.tsx`, and the verify page. 19 registrations across 18 distinct names; `explain_dimension` is shared by Studio and public profiles. Three rules hold this surface together. **The pre-standard `document.modelContext` global is confined to the adapter** — every tool imports the adapter instead, so a spec change is a one-file change. **A tool is registered against the view's lifecycle**, not once at load, so the catalog always describes the page actually on screen. **`SITE_TOOL_MAP` is a contract, not a comment**: `site-tool-map.test.ts` reads the four registration files and fails when the published map and the real registrations disagree, which is what stops the landing page's own answer to "what can I do here" from going stale. The whole surface is gated by `NEXT_PUBLIC_WEBMCP_ENABLED` and the `webmcp_enabled` DB flag. Full catalog and design methodology: `docs/webmcp.md`; editing checklist: `.claude/rules/webmcp.md`.
...
- `/u/:handle` shows badge + breakdown + embed snippet, sourced from the receipt view model, or a scoring-status panel (`collecting` / `action_needed`) when no receipt exists yet, or a "not on Chapa yet" state for an unregistered handle.
- Caching prevents repeated GitHub API calls for same handle within 24h.
- v7.2 explains recorded evidence/coverage without confidence deductions; there is no confidence percentage or penalty-flag display anywhere (#1335 phase 5 deleted `redactImpactForVisitor()` and the confidence types along with it).
- Repo contains `docs/impact-v7.md` (current spec truth), `docs/impact-v6.md` (retired, historical reference only), `docs/impact-v4.md`, `docs/impact-v5.md`, and `docs/svg-design.md`.
...
- Collection progress and failures are always observable: an owner sees a reason and a recovery path (`/api/scoring/status`, retry); an operator sees `/api/health`'s `scoringQueue` block and the `scoring_collection_failed` / `scoring_issuance_failed` / `scoring_queue_stuck` alerts. Nothing fails silently into a skipped receipt (#1335).
...
- **Bundle-size budget**: the largest JS chunk must stay under 350 KB, enforced by `scripts/check-bundle-size.sh` in the `analyze` job — against the build that actually ships. It previously ran only under `ANALYZE=true`, measuring output production never emits, so a 368 KB chunk passed CI indefinitely (#1319). One documented exception exists (the lazily imported AI insights parser), identified by content signature rather than by its content-hashed filename and bounded by its own 400 KB ceiling; see `docs/accepted-risks.md`. A second exception would mean the rule is wrong, not the chunk.
...
6. **Pure functions for scoring** — Impact v7.2 compute and normalization must be pure functions with deterministic output for a given input. This makes them trivially testable.
