-- Reject every PostgreSQL whitespace-only lease token. Migration 030 used
-- btrim(text), which strips spaces but leaves tabs and other whitespace.
...
-- Apply every provider outcome for a non-empty, non-whitespace lease, or none.
