#!/usr/bin/env bash
# Chapa Performance Agent — Weekly build & bundle analysis.
# Runs via launchd (com.chapa.performance-agent) or manually.
...
MODEL="sonnet"
OUTPUT_FILE="${CHAPA_DIR}/docs/agents/performance-report.md"
LOG_FILE="${LOGS_DIR}/performance-agent-$(date '+%Y-%m-%d').log"

log_info "=== Performance Agent starting ==="
...
publish_report_file "${TMP_OUTPUT}" "${OUTPUT_FILE}" "performance-agent"
trap - EXIT
...
log_info "=== Performance Agent complete ==="
