#!/usr/bin/env bash
# Chapa Security Scanner — Weekly security audit.
# Runs via launchd (com.chapa.security-agent) or manually.
...
log_info "=== Security Scanner starting ==="
...
publish_report_file "${TMP_OUTPUT}" "${OUTPUT_FILE}" "security-agent"
trap - EXIT
...
log_info "=== Security Scanner complete ==="
