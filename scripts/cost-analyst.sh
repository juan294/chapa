#!/usr/bin/env bash
# Chapa Cost Analyst — Daily infrastructure cost & usage analysis.
# Runs via launchd (com.chapa.cost-analyst) or manually.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
source "${SCRIPT_DIR}/lib/agent-utils.sh"

AGENT_KEY="cost_analyst"
MODEL="claude-haiku-4-5-20251001"
OUTPUT_FILE="${CHAPA_DIR}/docs/agents/cost-analyst-report.md"
LOG_FILE="${LOGS_DIR}/cost-analyst-$(date '+%Y-%m-%d').log"

log_info "=== Cost Analyst starting ==="
...
publish_report_file "${TMP_OUTPUT}" "${OUTPUT_FILE}" "cost-analyst"
trap - EXIT
...
log_info "=== Cost Analyst complete ==="
