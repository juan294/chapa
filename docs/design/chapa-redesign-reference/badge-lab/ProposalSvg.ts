// ISOLATED DESIGN CANDIDATE ice-terminal-v2-proposal. Derived from jade-v1.
...
import { getTierColor, getArchetypeColor } from "/Users/juan/code/chapa/apps/web/lib/render/theme";
import { buildHeatmapCells, renderHeatmapSvg } from "/Users/juan/code/chapa/apps/web/lib/render/heatmap";
import { renderBadgeBranding } from "/Users/juan/code/chapa/apps/web/lib/render/BadgeBranding";
import { renderRadarChart, type RadarChartLabels } from "/Users/juan/code/chapa/apps/web/lib/render/RadarChart";
import { escapeXml } from "/Users/juan/code/chapa/apps/web/lib/render/escape";
import {
  ...
  renderScoreEffect,
  renderTierTreatment,
} from "/Users/juan/code/chapa/apps/web/lib/render/badge-effects";
import { renderVerificationStrip, renderDemoVerificationStrip } from "/Users/juan/code/chapa/apps/web/lib/render/VerificationStrip";
import { VERIFICATION_CORAL } from "/Users/juan/code/chapa/apps/web/lib/badge-visual-metadata";
 ...
 */
export function renderProposalSvg(
  stats: StatsData,
  ...
  const t = proposalTheme(config.colorPalette);
  ...
  const heatmapX = PAD;
  const heatmapY = 246; // shifted down 30px for meta row
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}" data-badge-design="ice-terminal-v2-proposal"${a11yAttrs}>${a11yMarkup}
  <defs>
    <style>
  ...
  <!-- Background -->
  ${backgroundEffect.markup.replaceAll('rx="20"', 'rx="4"')}${cardStyleEffect.markup ? `
  ${cardStyleEffect.markup.replaceAll('rx="20"', 'rx="4"')}` : ""}${borderEffect.markup ? `
  ${borderEffect.markup.replaceAll('rx="19"', 'rx="3"')}` : ""}

  <!-- Terminal frame — visual only, all data builders remain shared. -->
  <line x1="60" y1="134" x2="1120" y2="134" stroke="${t.stroke}"/>
  <line x1="681" y1="211" x2="681" y2="540" stroke="${t.stroke}"/>
  <text x="60" y="230" font-family="'JetBrains Mono', monospace" font-size="14" fill="${t.textSecondary}" letter-spacing="1">01 / ACTIVITY</text>
  <text x="604" y="230" text-anchor="end" font-family="'JetBrains Mono', monospace" font-size="12" fill="${t.textSecondary}">13 WEEKS × 7 DAYS</text>
  <text x="719" y="230" font-family="'JetBrains Mono', monospace" font-size="14" fill="${t.textSecondary}" letter-spacing="1">02 / IMPACT</text>
  <!-- ─── Header row ─────────────────────────────────────── -->
  ...
  <!-- Handle -->
  <text x="${PAD + 72}" y="${headerY - 6}" font-family="'JetBrains Mono', monospace" font-size="32" font-weight="700" fill="${t.textPrimary}">${headerName}</text>
  ...
  <text ${disableAnimation ? '' : 'class="badge-score-pulse"'} x="${radarCX}" y="${ringCY}" font-family="'JetBrains Mono', monospace" font-size="52" font-weight="700" fill="${scoreEffect.fill}"${scoreEffect.attrs} text-anchor="middle" dominant-baseline="central">${scoreStr}</text>
  ...
  ${brandingSvg.replaceAll('#9AA4B2', t.textSecondary).replaceAll('opacity="0.5"', 'opacity="0.85"').replaceAll('opacity="0.8"', 'opacity="1"')}

  <!-- Verification seal (right edge) -->
  ${verificationSvg.replaceAll('opacity="0.9"', 'opacity="1"')}
</svg>`;
}
