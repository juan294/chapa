...
**Important caveat:** Many Bitbucket teams disable the built-in issue tracker in favor of Jira. The `has_issues` repo flag indicates availability. When issues are disabled, `issuesClosedCount` would be 0 — this naturally produces a lower Building score but is accurate.
