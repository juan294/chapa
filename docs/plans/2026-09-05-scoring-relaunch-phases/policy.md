...
Status: approved for implementation of all phases on 2026-09-05; empirical validation remains pending. The user explicitly approved four fixed core dimensions, optional separate Craft, and no penalty for AI usage, report disclosure or tool choice. Missing coverage uses the recommended explicit range policy; no midpoint is invented.
