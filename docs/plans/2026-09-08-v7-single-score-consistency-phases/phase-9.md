...
The implemented launcher is `scripts/quality/local-scoring-qualification.ts --root ROOT --evidence-dir IGNORED_DIR --port 3217` (run with `pnpm exec tsx --tsconfig apps/web/tsconfig.json`). It requires the verified `RELEASE_BUILD_MANIFEST`, the explicit disposable acknowledgment and dedicated loopback Supabase55331/Redis REST56380 credentials supplied only to the process. After verifying the compiled artifact manifest it discards only the mutable fetch cache, so build-time missing-service flags cannot shadow the fixture flags. It seeds synthetic owners, uses real Redis, denies unexpected provider fetches and records any remaining cache keys before the task-owned services are disposed. Existing local stacks are not qualification targets. For the separately named stack, run `pnpm run test:contract` with its local service environment and an absolute `CONTRACT_SUPABASE_CONFIG` pointing to its `supabase/config.toml`; every direct SQL contract must use the shared guard that binds the HTTP port, project identity and exact running container. The default `test:contract:local` command remains appropriate only for the root-config local stack.
